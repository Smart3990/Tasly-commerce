...
class FFAppState extends ChangeNotifier {
  ...
  List<String> _SearchHistory = ['Cardiotonic Pill', 'Phytoestrogen'];
  ...
  List<CartProductStruct> _cartItems = [
    CartProductStruct.fromSerializableMap(jsonDecode(
        '{\"product_ref\":\"/product/example\",\"name\":\"Hello World\",\"image\":\"https://picsum.photos/seed/268/600\",\"DistributorPrice\":\"0\",\"price\":\"0\",\"count\":\"0\",\"productID\":\"Hello World\"}'))
  ];
  List<CartProductStruct> get cartItems => _cartItems;
  set cartItems(List<CartProductStruct> value) {
    _cartItems = value;
    prefs.setStringList(
        'ff_cartItems', value.map((x) => x.serialize()).toList());
  }

  void addToCartItems(CartProductStruct value) {
    cartItems.add(value);
    prefs.setStringList(
        'ff_cartItems', _cartItems.map((x) => x.serialize()).toList());
  }

  void removeFromCartItems(CartProductStruct value) {
  ...
  void insertAtIndexInCartItems(int index, CartProductStruct value) {
  ...
  bool get isDistributor => _isDistributor;
