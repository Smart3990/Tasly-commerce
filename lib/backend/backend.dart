...
import 'schema/request_pin_record.dart';
import 'dart:async';
...
export 'schema/request_pin_record.dart';
...
/// Functions to query RequestPinRecords (as a Stream and as a Future).
Future<int> queryRequestPinRecordCount({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      RequestPinRecord.collection(parent),
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<RequestPinRecord>> queryRequestPinRecord({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      RequestPinRecord.collection(parent),
      RequestPinRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<RequestPinRecord>> queryRequestPinRecordOnce({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
...
Future<FFFirestorePage<RequestPinRecord>> queryRequestPinRecordPage({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, RequestPinRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      RequestPinRecord.collection(parent),
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<RequestPinRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
